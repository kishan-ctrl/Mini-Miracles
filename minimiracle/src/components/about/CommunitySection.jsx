...
const CommunitySection = ({ community }) => {
  const cards = [
    { title: "PTA & Feedback", icon: Users, desc: "Regular sessions to support student growth." },
    { title: "School Events", icon: CalendarDays, desc: "Concerts, sports meets, and celebration days." },
    { title: "Volunteering", icon: HandHeart, desc: "Parents can help with trips, events, and activities." },
  ];
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-14 sm:px-6">
      ...
      >
        <h2 className="text-2xl font-extrabold text-gray-900">{community.title}</h2>
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 lg:grid-cols-2">
        <div className="rounded-3xl bg-white p-6 ring-1 ring-gray-100 shadow-sm">
          <h3 className="text-base font-bold text-gray-900">How parents can participate</h3>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3 lg:grid-cols-1">
          {cards.map(({ title, icon: Icon, desc }) => (
            <div
              key={title}
              className="rounded-3xl bg-gray-50 p-6 ring-1 ring-gray-100"
            >
